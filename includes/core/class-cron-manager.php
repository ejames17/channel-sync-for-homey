...
class Homey_Channel_Sync_Cron {
	...
	public function __construct() {
		$this->options = get_option( 'homey_channel_sync_options', [] );

		// Hook WP-Cron schedules and actions
		add_filter( 'cron_schedules', [ $this, 'filter_cron_schedules' ] );
		add_action( 'homey_channel_sync_cron_hook', [ $this, 'run_synchronization' ] );

		// Register Manual AJAX execution endpoint
		add_action( 'wp_ajax_homey_sync_run_now', [ $this, 'ajax_run_now' ] );
	}
	...
	public function filter_cron_schedules( array $schedules ): array {
		if ( ! isset( $schedules['monthly'] ) ) {
			$schedules['monthly'] = [
				...
				'display'  => esc_html__( 'Once Monthly', 'homey-channel-sync' ),
			];
		}
		return $schedules;
	}
	...
	public function run_synchronization(): array {
		...
		if ( '1' !== $price_sync_enabled ) {
			return [
				'success' => false,
				'updated' => 0,
				'message' => esc_html__( 'Dynamic Price Sync feature is disabled in configuration.', 'homey-channel-sync' ),
			];
		}
		...
		if ( empty( $listings ) ) {
			return [
				'success' => true,
				'updated' => 0,
				'message' => esc_html__( 'No published Homey listings found to map.', 'homey-channel-sync' ),
			];
		}
		...
		if ( empty( $mappings ) ) {
			return [
				'success' => false,
				'updated' => 0,
				'message' => esc_html__( 'No mappings have been configured for listing records.', 'homey-channel-sync' ),
			];
		}
		...
		if ( 'beds24' === $active_channel ) {
			...
			if ( empty( $token ) ) {
				return [
					'success' => false,
					'updated' => 0,
					'message' => esc_html__( 'Synchronization aborted: No valid Beds24 access token. Please authenticate in plugin settings.', 'homey-channel-sync' ),
				];
			}
		}
		...
		if ( ! $adapter ) {
			return [
				'success' => false,
				'updated' => 0,
				'message' => esc_html__( 'Selected Channel Manager is currently unavailable.', 'homey-channel-sync' ),
			];
		}
		...
		$sync_logs = get_option( 'homey_sync_execution_logs', [] );
		...
		return [
			...
			'message' => sprintf(
				...
				esc_html__( 'Rates synchronized successfully! Updated %1$d listings in %2$s seconds.', 'homey-channel-sync' ),
				$updated_count,
				(string) $time_elapsed
			),
		];
	...
	public function ajax_run_now(): void {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( [ 'message' => esc_html__( 'Unauthorized permissions context.', 'homey-channel-sync' ) ] );
		}
		...
		if ( $result['success'] ) {
			wp_send_json_success( [
				...
				'details' => sprintf(
					...
					esc_html__( 'Database write sequence completed for %d entities.', 'homey-channel-sync' ),
					$result['updated']
				),
			] );
		} else {
			wp_send_json_error( [
				'message' => esc_html__( 'Rate synchronization failed.', 'homey-channel-sync' ),
