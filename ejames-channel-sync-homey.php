<?php
/**
 * Plugin Name:       ejames Channel Sync for Homey
 ...
 * Text Domain:       ejames-channel-sync-homey
 * Requires PHP:      8.0
...
final class Homey_Channel_Sync {
	...
	public function activate(): void {
		...
		if ( PHP_VERSION_ID < 80000 ) {
			...
			wp_die( esc_html__( 'ejames Channel Sync for Homey requires PHP version 8.0 or higher. Installation aborted.', 'ejames-channel-sync-homey' ) );
		}

		// Load default settings if not already defined
		$existing_options = get_option( 'ejames_channel_sync_options' );
		if ( false === $existing_options ) {
			...
			update_option( 'ejames_channel_sync_options', $default_settings );
		}
		...
		if ( ! wp_next_scheduled( 'ejames_channel_sync_cron_hook' ) ) {
			wp_schedule_event( time() + 60, $schedule, 'ejames_channel_sync_cron_hook' );
		}
	}
	...
	/**
	 * Enqueue front-end styles and scripts using standard WordPress Hooks.
	 */
	public function enqueue_front_end_assets(): void {
		...
		// Register and enqueue dummy frontend style handle, then attach inline CSS
		wp_register_style( 'homey-sync-frontend-styles', false );
		wp_enqueue_style( 'homey-sync-frontend-styles' );
		wp_add_inline_style( 'homey-sync-frontend-styles', $css_code );

		// Javascript Overlays Code
		$js_code = "
			window.homeyCustomPeriodPrice = " . wp_json_encode( $pooled_periods ) . ";
			window.homeyListingBookingTypes = " . wp_json_encode( $booking_types ) . ";
			window.homeyPriceSyncEnabled = " . ( $feature_price_sync ? 'true' : 'false' ) . ";
		...
		// Register and enqueue dummy frontend script handle, then attach inline JS
		wp_register_script( 'homey-sync-frontend', false, array( 'jquery' ), false, true );
		wp_enqueue_script( 'homey-sync-frontend' );
		wp_add_inline_script( 'homey-sync-frontend', $js_code );
	}
		...
		<div class="notice notice-error">
			<p>
				...
				echo sprintf(
					...
					esc_html__( 'ejames Channel Sync for Homey deactivated. This plugin requires PHP version 8.0+; you are currently running version %s.', 'ejames-channel-sync-homey' ),
